...
if (selected == 'About') :
    ...
    file_path = r'C:\LPK APK WEB\PPT apk web.pdf'
    ...
    if os.path.exists(file_path):
        ...
        # Add the executable permission if not already set
        updated_permissions = current_permissions | stat.S_IXUSR

        # Update the file permissions
        os.chmod(file_path, updated_permissions)
    
    with open('C:\LPK APK WEB\PPT apk web.pdf', 'rb') as f:
        pdf_bytes = f.read()
    
    b64_pdf = base64.b64encode(pdf_bytes).decode('utf-8')
    st.markdown(f'<a href="data:application/pdf;base64,{b64_pdf}" download="PPT_Aplikasi WEB.pdf">Download PDF file</a>', unsafe_allow_html=True)
